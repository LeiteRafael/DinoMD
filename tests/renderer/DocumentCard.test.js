...
describe('DocumentCard', () => {
    ...
    test('renders the document name', () => {
        render(<DocumentCard id="1" name="My Note" status="available" onClick={vi.fn()} />)
    ...
    test('shows a "Missing" badge when status is missing', () => {
        render(<DocumentCard id="1" name="Gone" status="missing" onClick={vi.fn()} />)

        expect(screen.getByText('Missing')).toBeInTheDocument()
    })

    test('does NOT show a badge when status is available', () => {
        render(<DocumentCard id="1" name="OK" status="available" onClick={vi.fn()} />)

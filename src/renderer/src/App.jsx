...
export default function App() {
  ...
  // T008a: sort documents by mtimeMs descending for the sidebar
  const sortedDocuments = [...(docsHook.documents ?? [])].sort(
    (a, b) => (b.mtimeMs ?? 0) - (a.mtimeMs ?? 0)
  )

  // T023: if the active document is deleted while open, reset to main
  useEffect(() => {
  ...
  // ── After successful rename (T020b) ─────────────────────────────────────────
  function handleDocumentRenamed() {
    docsHook.refreshDocuments()
  }

  // ── Sidebar opening — open in reader ────────────────────────────────────────
  ...
  function withSidebar(pageNode) {
    ...
    if (!sidebarHook.open) {
      return (
        <div style={{ display: 'flex', height: '100%', width: '100%' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>{pageNode}</div>
        </div>
      )
    }
    ...
    return (
      <PanelGroup direction="horizontal" style={{ height: '100%', width: '100%' }}>
        ...
        <PanelResizeHandle style={{ width: '4px', background: '#313244', cursor: 'col-resize' }} />

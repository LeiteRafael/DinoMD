// Tests for useEditor hook — isDirty, session transitions, save/discard/rename/delete
...
describe('openNew', () => {
  test('creates a draft session with empty content and isDraft=true', async () => {
    api.create.mockResolvedValue({
      success: true,
      draft: { id: 'draft-001', name: 'Untitled', filePath: null, isDraft: true },
      error: null
    })
  ...
  test('isDirty is false after openNew (content equals savedContent)', async () => {
    ...
    const { result } = renderHook(() => useEditor())
    await act(async () => { await result.current.openNew() })

    expect(result.current.isDirty).toBe(false)
  })

  test('sets error state when api.create fails', async () => {
    api.create.mockResolvedValue({ success: false, draft: null, error: 'API error' })
...
describe('openExisting', () => {
  const doc = { id: 'doc-001', name: 'My Notes', filePath: '/notes/my-notes.md', mtimeMs: 1700000 }

  test('loads content from disk and seeds savedContent', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '# My Notes\nHello', error: null })

    const { result } = renderHook(() => useEditor())
    await act(async () => { await result.current.openExisting(doc) })
  ...
  test('isDirty is false immediately after openExisting', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '# Loaded', error: null })
...
describe('isDirty', () => {
  async function openedHook() {
    ...
    await act(async () => { await r.result.current.openNew() })
    return r
  }
  ...
  test('becomes true after updateContent changes content', async () => {
    ...
    act(() => { result.current.updateContent('some new content') })
  ...
  test('becomes false again when content is reset to savedContent', async () => {
    ...
    act(() => { result.current.updateContent('') }) // back to empty savedContent
    expect(result.current.isDirty).toBe(false)
  })
})
...
describe('save', () => {
  test('saves a draft: updates session with resolved path/name and clears isDraft', async () => {
    ...
    api.save.mockResolvedValue({
      ...
      filePath: '/notes/doc.md', name: 'doc', mtimeMs: 1700000, error: null
    })

    const { result } = renderHook(() => useEditor())
    await act(async () => { await result.current.openNew() })
    act(() => { result.current.updateContent('# New doc') })
  ...
  test('returns canceled=true without changing state when dialog is dismissed', async () => {
    ...
    api.save.mockResolvedValue({ success: true, canceled: true, filePath: null, name: null, mtimeMs: null, error: null })

    const { result } = renderHook(() => useEditor())
    await act(async () => { await result.current.openNew() })
    act(() => { result.current.updateContent('something') })
    ...
    expect(result.current.session.isDraft).toBe(true) // still a draft
  ...
  test('sets error state and returns saved=false when api.save reports failure', async () => {
    ...
    api.save.mockResolvedValue({ success: false, canceled: false, error: 'EACCES: permission denied' })
...
describe('discard', () => {
  test('resets content to savedContent, making isDirty false', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '# Original', error: null })

    const { result } = renderHook(() => useEditor())
    await act(async () => {
      await result.current.openExisting({ id: 'doc-001', name: 'Doc', filePath: '/doc.md', mtimeMs: null })
    })
...
describe('rename', () => {
  test('updates session name and filePath on success', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '# Doc', error: null })
    api.rename.mockResolvedValue({ success: true, newFilePath: '/notes/new-name.md', error: null })

    const { result } = renderHook(() => useEditor())
    await act(async () => {
      await result.current.openExisting({ id: 'doc-001', name: 'old-name', filePath: '/notes/old-name.md', mtimeMs: null })
    })

    await act(async () => { await result.current.rename('new-name') })
  ...
  test('sets error state when rename fails (e.g. name conflict)', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '# Doc', error: null })
    api.rename.mockResolvedValue({ success: false, newFilePath: null, error: 'A document with that name already exists.' })
    ...
    await act(async () => { await result.current.rename('conflicting-name') })

    expect(result.current.error).toMatch(/already exists/i)
    expect(result.current.session.name).toBe('old-name') // unchanged
  })
})
...
describe('deleteDocument', () => {
  test('calls api.delete with the document id', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '', error: null })
    api.delete.mockResolvedValue({ success: true, canForceDelete: false, error: null })

    const { result } = renderHook(() => useEditor())
    await act(async () => {
      await result.current.openExisting({ id: 'doc-001', name: 'My Doc', filePath: '/doc.md', mtimeMs: null })
    })

    let deleteResult
    await act(async () => { deleteResult = await result.current.deleteDocument() })

    expect(api.delete).toHaveBeenCalledWith({ id: 'doc-001', force: false })
  ...
  test('passes force=true to api.delete when called with true', async () => {
    ...
    await act(async () => { await result.current.deleteDocument(true) })

    expect(api.delete).toHaveBeenCalledWith({ id: 'doc-001', force: true })
  })

  test('sets error state when delete fails', async () => {
    api.readContent.mockResolvedValue({ success: true, content: '', error: null })
    api.delete.mockResolvedValue({ success: false, canForceDelete: true, error: 'Trash unavailable' })
    ...
    await act(async () => { await result.current.deleteDocument() })
